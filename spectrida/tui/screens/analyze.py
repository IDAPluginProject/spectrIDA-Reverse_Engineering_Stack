...
_SHARD_DONE = re.compile(r"\[shard [^\]]+\]\s+(\d+) funcs exported")
_FUNCS      = re.compile(r"(\d[\d,]*) funcs")
_LAUNCH     = re.compile(r"launching (\d+) workers")
_PROG_BAR   = re.compile(r"\[[\#\-]{4,}\]")   # progress bar line — parse but don't log
...
class AnalyzeScreen(Screen):
        ...
        async def _wrap():
            try:
                ...
                try:
                    ...
                    log.write(Text(f"  ERROR: {exc}", Style(color="#ef4444")))
                    log.write(Text(f"  {traceback.format_exc()}", Style(color="#ef4444")))
                except Exception:
                    pass
    ...
    async def _run(self) -> None:
        ...
        async def on_line(line: str) -> None:
            if not _PROG_BAR.search(line):
                log.write(Text(f"  {line}", Style(color="#64748b")))
            if (m := _LAUNCH.search(line)):
                n = min(int(m.group(1)), self._workers)
                for i in range(n):
                    self._set_shard(i, "running")
            if _SHARD_DONE.search(line):
                self._set_shard(self._shards_done, "done")
                self._shards_done += 1
            if (m := _FUNCS.search(line)):
                self._n_funcs = max(self._n_funcs, int(m.group(1).replace(",", "")))
            stats.update(
                f"  {self._shards_done}/{self._workers} shards  │  {self._n_funcs:,} functions found")
            await asyncio.sleep(1 / 60)  # one compositor frame so Textual actually renders
